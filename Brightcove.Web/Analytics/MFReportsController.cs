...
namespace Sitecore.MediaFramework.Analytics.Dashboard
{
    ...
    public class MFReportsController : ServicesApiController
    {
        ...
        public IHttpActionResult Get(string datasource, string siteName)
        {
            ...
            string reportSQLQuery = "";

            using (new SecurityDisabler())
            {
                var database = Database.GetDatabase("core");
                Item dataSourceItem = database.GetItem(new ID(datasource));
                reportSQLQuery = dataSourceItem.Fields["{0AA8B742-BBDF-4405-AB8D-6FAC7E79433B}"].Value;
            }
